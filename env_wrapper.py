...
class ProcgenWrapper():
    """
    MaxLast2Frame + LifeLossInfo for Procgen
    """
    def __init__(self, env, skip=4):
        super().__init__()
        self.env = env
    ...
    def step(self, action):
        ...
        info[0]["life_loss"] = False
        info[0]["episode_frame_number"] = self.frame_num

        if len(self.obs_buffer) == 1:
            obs = self.obs_buffer[0]
        else:
            obs = np.max(np.stack(self.obs_buffer), axis=0)
        return obs, reward, terminated, truncated, info[0]
    ...
    def reset(self, **kwargs):
        ...
        info = {}
        info["life_loss"] = False
        info["episode_frame_number"] = np.array([0])
        return obs, info


class ProcgenGym3Wrapper(gym3.Wrapper):
    def __init__(self, env, skip=4):
        super().__init__(env)
        self.env = env
        self.n = env.num
        self.skip = skip
        self.frame_num = np.array([0] * self.n)
    ...
    def reset(self):
        ...
        info = {}
        info["life_loss"] = False
        info["episode_frame_number"] = np.array([0] * self.n)
        return obs, info
    
    def step(self, action):
        self.env.act(action)
        reward, observation, terminated = self.env.observe()
        obs = observation["rgb"]
        for i in range(self.n):
            if terminated[i]:
                self.frame_num[i] = 0
            else:
                self.frame_num[i] += self.skip
        
        info = {}
        info["life_loss"] = False
        info["episode_frame_number"] = self.frame_num

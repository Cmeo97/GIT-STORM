...
if __name__ == "__main__":
    ...
    import train
    dummy_env = build_single_env(args.env_name, conf.BasicSettings.ImageSize, 0)
    action_dim = dummy_env.action_space.n
    world_model = train.build_world_model(conf, action_dim)
    agent = train.build_agent(conf, action_dim)
    root_path = f"outputs/{args.run_name}/ckpt/{args.run_name}"
    ...
    for step in tqdm(steps):
        wm_dict =torch.load(f"{root_path}/world_model_{step}.pth") 
        #wm_dict.pop("maskgit.net.broadcast_position_biases.embs.0")
        world_model.load_state_dict(wm_dict)
    ...
    with open(f"outputs/eval_result/{args.run_name}_one_iteration.csv", "w") as fout:

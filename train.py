...
from sub_models.world_models import WorldModel, GITWorldModel, MSELoss
...
def build_single_env(env_name, image_size, seed):
    if "ALE" in env_name:
        ...
        # env = ProcgenEnv(num_envs=1, env_name=env_name[8:], num_levels=0, start_level=0, distribution_mode="easy")
        # env = env_wrapper.ProcgenWrapper(env)
        env = ProcgenGym3Env(num=1, env_name=env_name[8:], num_levels=0, start_level=0, distribution_mode="easy")
        env = env_wrapper.ProcgenGym3Wrapper(env)
    return env
...
def build_vec_env(env_name, image_size, num_envs, seed):
    if "ALE" in env_name:
        ...
        # vec_env = ProcgenEnv(num_envs=num_envs, env_name=env_name[8:], num_levels=0, start_level=0, distribution_mode="easy")
        # vec_env = env_wrapper.ProcgenWrapper(vec_env)
        vec_env = ProcgenGym3Env(num=num_envs, env_name=env_name[8:], num_levels=0, start_level=0, distribution_mode="easy")
        vec_env = env_wrapper.ProcgenGym3Wrapper(vec_env)
    return vec_env


def train_world_model_step(replay_buffer: ReplayBuffer, world_model: Union[WorldModel, GITWorldModel], batch_size, demonstration_batch_size, batch_length, logger):
...
def joint_train_world_model_agent(env_name, max_steps, num_envs, image_size,
                                  ...
                                  imagine_batch_size, imagine_demonstration_batch_size,
                                  imagine_context_length, imagine_batch_length,
                                  save_every_steps, seed, logger, name,
                                  silent):
    # create ckpt dir
    ...
    # sample and train
    for total_steps in tqdm(range(max_steps//num_envs), disable=silent):
        # sample part >>>
            ...
            with torch.no_grad():
                if len(context_action) == 0:
                    if "ALE" in env_name:
                        ...
                        action = gym3.types_np.sample(vec_env.action_space, bshape=(vec_env.num,))
...
def main(conf: DictConfig):  
    ...
    if conf.Task == "JointTrainAgent":
        ...
        if "ALE" in conf.BasicSettings.env_name:
            action_dim = dummy_env.action_space.n
        elif "Procgen" in conf.BasicSettings.env_name:
            action_dim = len(dummy_env.env.combos)
        ...
        # if conf.BasicSettings.pretrained_path is not None:
        #     import glob
        #     root_path = f"{hydra.utils.get_original_cwd()}/{conf.BasicSettings.pretrained_path}"
        #     pathes = glob.glob(f"{root_path}/world_model_*.pth")
        #     steps = [int(path.split("_")[-1].split(".")[0]) for path in pathes]
        #     steps.sort()
        #     print(f"Loading from: {conf.BasicSettings.pretrained_path}")
        #     world_model.load_state_dict(torch.load(f"{root_path}/world_model_{steps[-1]}.pth"))
        #     agent.load_state_dict(torch.load(f"{root_path}/agent_{steps[-1]}.pth"))
        ...
        joint_train_world_model_agent(
            ...
            batch_size=conf.JointTrainAgent.BatchSize*conf.JointTrainAgent.NumEnvs,
            ...
            silent=conf.BasicSettings.silent
        )
    else:
